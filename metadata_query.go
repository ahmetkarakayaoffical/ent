...
func (mq *MetadataQuery) QueryOwner() *AgentQuery {
...
func (mq *MetadataQuery) WithOwner(opts ...func(*AgentQuery)) *MetadataQuery {
	query := (&AgentClient{config: mq.config}).Query()
...
func (mq *MetadataQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Metadata, error) {
	...
	if query := mq.withOwner; query != nil {
		if err := mq.loadOwner(ctx, query, nodes,
			func(n *Metadata) { n.Edges.Owner = []*Agent{} },
			func(n *Metadata, e *Agent) { n.Edges.Owner = append(n.Edges.Owner, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MetadataQuery) loadOwner(ctx context.Context, query *AgentQuery, nodes []*Metadata, init func(*Metadata), assign func(*Metadata, *Agent)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Metadata)
	nids := make(map[string]map[*Metadata]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(metadata.OwnerTable)
		s.Join(joinT).On(s.C(agent.FieldID), joinT.C(metadata.OwnerPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(metadata.OwnerPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(metadata.OwnerPrimaryKey[1]))
